...
def main():
    ...
    if args.plugin:
        ...
        buildConfig(script_dir, project_dir, args.cuda_path)
    ...
    if args.build_unity:
        ...
        unity_build_dir_gl = os.path.join(
            script_dir, 'build', 'UnityBenchmark-GL')
        os.makedirs(unity_build_dir_gl, exist_ok=True)
        subprocess.run([args.unityBin, '-projectPath', unity_dir, '-buildTarget', 'Win64', '-executeMethod',
        ...
        for i in range(args.number_tests):
            ...
            if os.path.exists(benchmark_gl_path):
                benchmark_gl = subprocess.run(
                    [benchmark_gl_path, '-batchmode' if args.batchmode else ''])
                    ...
                    for file in os.listdir(os.path.join(script_dir, 'build', 'UnityBenchmark-GL', 'UnityBenchmark_Data')):
                        if file.endswith('.csv'):
                            shutil.copy(os.path.join(
                                script_dir, 'build', 'UnityBenchmark-GL', 'UnityBenchmark_Data', file), results_dir_gl)
            else:
                print_red(
            ...
            if os.path.exists(benchmark_dx11_path):
                benchmark_dx11 = subprocess.run(
                    [benchmark_dx11_path, '-batchmode' if args.batchmode else ''])
                if benchmark_dx11.returncode == 0:
                    ...
                    for file in os.listdir(os.path.join(script_dir, 'build', 'UnityBenchmark-DX11', 'UnityBenchmark_Data')):
                        if file.endswith('.csv'):
                            shutil.copy(os.path.join(
                                script_dir, 'build', 'UnityBenchmark-DX11', 'UnityBenchmark_Data', file), results_dir_dx11)
            else:
                print_red(
